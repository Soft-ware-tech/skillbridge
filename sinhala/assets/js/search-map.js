...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (useMyLocationBtn) {
    useMyLocationBtn.addEventListener("click", function () {
      if (!navigator.geolocation) {
        alert("ඔබේ බ්‍රවුසරය ස්ථාන සෙවීමට සහාය නොදක්වයි.");
        return;
      }
      useMyLocationBtn.textContent = "ස්ථානගත කරමින්...";
  ...
  viewToggleBtn.addEventListener("click", function () {
    ...
    if (showingMap) {
      ...
      viewToggleBtn.textContent = "🗺️ සිතියම් දසුන";
      return;
    }
  ...
  function renderResultsMap() {
    ...
    if (points.length === 0) {
      resultsMapEl.textContent = "මෙම පිටුවේ නිදහස් වෘත්තිකයින් කිසිවෙකුට තවම සුරැකි ස්ථානයක් නොමැත.";
      return;
    }
