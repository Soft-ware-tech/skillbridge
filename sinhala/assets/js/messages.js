...
(function () {
  ...
  function formatLastSeen(lastSeenStr) {
    ...
    if (secondsAgo <= 60) return "සබැඳිව";

    var minutes = Math.floor(secondsAgo / 60);
    if (minutes < 60) {
      return (
        "අවසන් වරට දුටුවේ " + minutes + (minutes === 1 ? " min ago" : " mins ago")
      );
    }

    var hours = Math.floor(minutes / 60);
    if (hours < 24) {
      return "අවසන් වරට දුටුවේ " + hours + (hours === 1 ? " hr ago" : " hrs ago");
    }

    return "අවසන් වරට දුටුවේ " + formatTime(lastSeenStr);
  }
    ...
    fetch("messages-send.php", {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          alert(data.error || "එම පණිවිඩය යැවිය නොහැකි විය.");
        }
      })
      .catch(function () {
