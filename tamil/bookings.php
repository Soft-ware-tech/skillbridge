...
<title>முன்பதிவுகள் — SkillBridge.lk</title>
...
<main class="dash-page">
  ...
  <p class="eyebrow">முன்பதிவு கோரிக்கைகள்</p>
  <h1 class="dash-title">முன்பதிவுகள்</h1>

  <div class="filter-row">
    <a href="bookings.php" class="chip <?php echo $filter === '' ? 'is-active' : ''; ?>">அனைத்தும்</a>
    <a href="bookings.php?status=pending" class="chip <?php echo $filter === 'pending' ? 'is-active' : ''; ?>">நிலுவையில்</a>
    <a href="bookings.php?status=confirmed" class="chip <?php echo $filter === 'confirmed' ? 'is-active' : ''; ?>">உறுதிசெய்யப்பட்டது</a>
    <a href="bookings.php?status=completed" class="chip <?php echo $filter === 'completed' ? 'is-active' : ''; ?>">முடிக்கப்பட்டது</a>
    <a href="bookings.php?status=cancelled" class="chip <?php echo $filter === 'cancelled' ? 'is-active' : ''; ?>">ரத்து செய்யப்பட்டது</a>
  </div>

  <?php if ($dbError): ?>
    <div class="empty-state">
      <div class="empty-glyph">!</div>
      <h3>முன்பதிவுகளை ஏற்ற முடியவில்லை</h3>
  ...
  <?php elseif (empty($bookings)): ?>
    <div class="empty-state">
      ...
      <p>இந்த வடிகட்டலுடன் எதுவும் பொருந்தவில்லை.</p>
    </div>
          ...
          <div class="table-row-main">
            ...
            <?php if ($bk['latitude'] !== null && $bk['longitude'] !== null): ?>
              <button type="button"
                ...
                📍 வாடிக்கையாளர் இருப்பிடத்தைக் காண்க
              </button>
          ...
          <div class="table-row-actions">
            ...
            <?php if ($bk['status'] === 'confirmed'): ?>
              <form method="POST" action="booking-status-update.php">
                ...
                <button type="submit" class="btn btn-outline btn-sm">முடிந்ததாக குறியிடு</button>
              </form>
            <?php endif; ?>
            ...
            <?php if (in_array($bk['status'], ['pending', 'confirmed'], true)): ?>
              <form method="POST" action="booking-status-update.php" onsubmit="return confirm('Cancel this booking?');">
                ...
                <button type="submit" class="btn btn-outline btn-sm btn-danger">ரத்து செய்</button>
              </form>
            <?php endif; ?>
          </div>
        </div>
