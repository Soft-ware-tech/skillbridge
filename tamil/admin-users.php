...
<title>பயனர்கள் — SkillBridge.lk</title>
...
<main class="dash-page">
  ...
  <p class="eyebrow">அனைத்து கணக்குகள்</p>
  <h1 class="dash-title">பயனர்கள்</h1>
  ...
  <form class="admin-search-bar" method="GET" action="admin-users.php">
    ...
    <input type="text" name="q" value="<?php echo htmlspecialchars($search, ENT_QUOTES); ?>" placeholder="பெயர் அல்லது மின்னஞ்சல் மூலம் தேடுங்கள்...">
    <button type="submit" class="btn btn-outline">தேடு</button>
  </form>

  <div class="filter-row">
    <a href="admin-users.php<?php echo $search !== '' ? '?q=' . urlencode($search) : ''; ?>" class="chip <?php echo $roleFilter === '' ? 'is-active' : ''; ?>">அனைத்தும்</a>
    <a href="admin-users.php?role=client<?php echo $search !== '' ? '&q=' . urlencode($search) : ''; ?>" class="chip <?php echo $roleFilter === 'client' ? 'is-active' : ''; ?>">வாடிக்கையாளர்கள்</a>
    <a href="admin-users.php?role=freelancer<?php echo $search !== '' ? '&q=' . urlencode($search) : ''; ?>" class="chip <?php echo $roleFilter === 'freelancer' ? 'is-active' : ''; ?>">ஃப்ரீலான்சர்கள்</a>
    <a href="admin-users.php?role=admin<?php echo $search !== '' ? '&q=' . urlencode($search) : ''; ?>" class="chip <?php echo $roleFilter === 'admin' ? 'is-active' : ''; ?>">நிர்வாகிகள்</a>
  </div>

  <?php if ($dbError): ?>
    <div class="empty-state">
      <div class="empty-glyph">!</div>
      <h3>பயனர்களை ஏற்ற முடியவில்லை</h3>
  ...
  <?php elseif (empty($users)): ?>
    <div class="empty-state">
      <div class="empty-glyph">◎</div>
      <h3>பயனர்கள் யாரும் கிடைக்கவில்லை</h3>
      <p>இந்தத் தேடல் அல்லது வடிகட்டலுடன் எதுவும் பொருந்தவில்லை.</p>
    </div>
        ...
        <div class="table-row <?php echo (int) $u['is_active'] === 0 ? 'is-inactive' : ''; ?>">
          <div class="table-row-main">
            ...
            <span class="table-meta">
              ...
              &middot; இணைந்த நாள் <?php echo htmlspecialchars(date('d M Y', strtotime($u['created_at'])), ENT_QUOTES); ?>
              <?php if ((int) $u['is_active'] === 0): ?> &middot; <span class="paused-tag">செயலிழக்கச் செய்யப்பட்டது</span><?php endif; ?>
            </span>
          </div>
