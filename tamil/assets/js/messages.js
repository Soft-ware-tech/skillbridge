...
(function () {
  ...
  function formatLastSeen(lastSeenStr) {
    ...
    if (secondsAgo <= 60) return "ஆன்லைனில்";

    var minutes = Math.floor(secondsAgo / 60);
    if (minutes < 60) {
      return (
        "கடைசியாகப் பார்க்கப்பட்டது " + minutes + (minutes === 1 ? " min ago" : " mins ago")
      );
    }

    var hours = Math.floor(minutes / 60);
    if (hours < 24) {
      return "கடைசியாகப் பார்க்கப்பட்டது " + hours + (hours === 1 ? " hr ago" : " hrs ago");
    }

    return "கடைசியாகப் பார்க்கப்பட்டது " + formatTime(lastSeenStr);
  }
    ...
    fetch("messages-send.php", {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          alert(data.error || "அந்த செய்தியை அனுப்ப முடியவில்லை.");
        }
      })
      .catch(function () {
