/* ==========================================================================
   SkillBridge.lk -- Search page: "அருகில்" geolocation + results map toggle
   Leaflet.js + OpenStreetMap tiles (free, no API key). Two independent
   features:
     1. "எனக்கு அருகில் தேடு" -- browser Geolocation API (always free, nothing
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ---- 1. "எனக்கு அருகில் தேடு" ----
  var useMyLocationBtn = document.getElementById("useMyLocationBtn");
  if (useMyLocationBtn) {
    useMyLocationBtn.addEventListener("click", function () {
      if (!navigator.geolocation) {
        alert("உங்கள் உலாவி இருப்பிடத் தேடலை ஆதரிக்கவில்லை.");
        return;
      }
      useMyLocationBtn.textContent = "இருப்பிடத்தைக் கண்டறிகிறது...";
  ...
  viewToggleBtn.addEventListener("click", function () {
    ...
    if (showingMap) {
      ...
      viewToggleBtn.textContent = "🗺️ வரைபடக் காட்சி";
      return;
    }
  ...
  function renderResultsMap() {
    ...
    if (points.length === 0) {
      resultsMapEl.textContent = "இந்தப் பக்கத்தில் உள்ள ஃப்ரீலான்சர்கள் யாருக்கும் இதுவரை சேமிக்கப்பட்ட இருப்பிடம் இல்லை.";
      return;
    }
