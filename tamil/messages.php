...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $withId > 0) {
    ...
    if (empty($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $submittedToken)) {
        ...
        if ($content === '') {
            $formError = 'அனுப்புவதற்கு முன் ஒரு செய்தியை தட்டச்சு செய்யவும்.';
        } else {
            try {
                $insert = $pdo->prepare("
                ...
                $formError = 'அதை அனுப்புவதில் ஏதோ தவறு நடந்தது. மீண்டும் முயற்சிக்கவும்.';
            }
        }
    }
}
...
{
    if (!$lastSeen) {
        return ['online' => false, 'label' => 'ஆஃப்லைனில்'];
    }

    $secondsAgo = time() - strtotime($lastSeen);

    if ($secondsAgo <= ONLINE_WINDOW_SECONDS) {
        return ['online' => true, 'label' => 'ஆன்லைனில்'];
    }

    $minutes = (int) floor($secondsAgo / 60);
    if ($minutes < 60) {
        $label = 'கடைசியாகப் பார்க்கப்பட்டது ' . $minutes . ' min' . ($minutes === 1 ? '' : 's') . ' ago';
        return ['online' => false, 'label' => $label];
    }

    $hours = (int) floor($minutes / 60);
    if ($hours < 24) {
        $label = 'கடைசியாகப் பார்க்கப்பட்டது ' . $hours . ' hr' . ($hours === 1 ? '' : 's') . ' ago';
        return ['online' => false, 'label' => $label];
    }

    $label = 'கடைசியாகப் பார்க்கப்பட்டது ' . date('d M, H:i', strtotime($lastSeen));
    return ['online' => false, 'label' => $label];
}
...
<title>செய்திகள் — SkillBridge.lk</title>
...
<main class="messages-page">
  ...
  <?php if ($dbError): ?>
    <div class="empty-state">
      ...
      <h3>செய்திகளை ஏற்ற முடியவில்லை</h3>
  ...
  <?php elseif ($withId > 0 && !$otherPerson): ?>
    <div class="empty-state">
      <div class="empty-glyph">◎</div>
      <h3>நபர் கிடைக்கவில்லை</h3>
      <a href="messages.php" class="btn btn-outline">செய்திகளுக்குத் திரும்பு</a>
    </div>
  ...
  <?php elseif ($withId > 0): ?>
    ...
    <div class="thread-shell">
      <div class="thread-header">
        <a href="messages.php" class="thread-back">← இன்பாக்ஸ்</a>
      ...
      <div class="thread-messages" id="threadMessages" data-with-id="<?php echo $withId; ?>" data-my-id="<?php echo $myId; ?>" data-last-id="<?php echo !empty($thread) ? (int) $thread[count($thread) - 1]['message_id'] : 0; ?>">
        <?php if (empty($thread)): ?>
          <p class="thread-empty">ஹலோ சொல்லுங்கள் — இது உங்கள் உரையாடலின் தொடக்கம்.</p>
        <?php else: ?>
      ...
      <form method="POST" action="messages.php?with=<?php echo $withId; ?>" class="thread-composer" id="composerForm" data-with-id="<?php echo $withId; ?>">
        ...
        <input type="text" name="content" placeholder="செய்தியை தட்டச்சு செய்யவும்..." autocomplete="off" required>
  ...
  <?php else: ?>
    ...
    <!-- =========================== INBOX =========================== -->
    <p class="eyebrow">உரையாடல்கள்</p>
    <h1 class="messages-title">செய்திகள்</h1>

    <?php if (empty($inbox)): ?>
      <div class="empty-state">
        <div class="empty-glyph">◎</div>
        <h3>இதுவரை உரையாடல்கள் இல்லை</h3>
        <p>உரையாடலைத் தொடங்க ஃப்ரீலான்சரின் சுயவிவரத்திலிருந்து செய்தி அனுப்புங்கள்.</p>
        <a href="search.php" class="btn btn-outline">ஃப்ரீலான்சரைத் தேடுங்கள்</a>
      </div>
    <?php else: ?>
