 ...
 * ---------------------------------------------------------------------
 * Every "Find a Freelancer" link, category-carousel card, and nav item
...
// "Nearby" search — the client's coordinates (dropped in via JS, see
...
$where  = ['s.is_active = 1', "s.language = 'english'"];
$params = [];
...
<title>Find a Freelancer — SkillBridge.lk</title>
...
<header class="nav">
  <div class="nav-inner">
    ...
    <nav class="nav-links">
      ...
      <a href="index(ENG).html#how-it-works">How it Works</a>
    </nav>
...
<main class="search-page">
  ...
  <section class="search-hero">
    <p class="eyebrow">Find A Freelancer</p>
    <h1 class="search-title">Search The Marketplace</h1>
    ...
    <form class="search-bar" action="search.php" method="GET">
      ...
      <?php if ($hasOrigin): ?>
        ...
        <?php endif; ?>
      <?php endif; ?>
      <input type="text" name="q" value="<?php echo htmlspecialchars($keyword, ENT_QUOTES); ?>" placeholder="Search by service, skill, or freelancer name...">
      <button type="submit" class="btn btn-primary">Search</button>
    </form>

    <div class="chip-row">
      <a href="<?php echo build_query(['category' => null, 'page' => null]); ?>" class="chip<?php echo $categoryKey === '' ? ' is-active' : ''; ?>">All</a>
    ...
    <div class="nearby-bar">
      <?php if (!$hasOrigin): ?>
        <button type="button" id="useMyLocationBtn" class="chip chip-nearby">📍 Search Near Me</button>
        <span class="nearby-hint">Uses your browser location to sort freelancers by distance.</span>
      <?php else: ?>
        <span class="nearby-active-tag">📍 Sorted near you</span>
        <div class="chip-row chip-row--inline">
          <a href="<?php echo build_query(['radius' => null, 'page' => null]); ?>" class="chip<?php echo $radiusKm === null ? ' is-active' : ''; ?>">Any distance</a>
          <?php foreach ([5, 10, 25, 50] as $r): ?>
            <a href="<?php echo build_query(['radius' => $r, 'page' => null]); ?>" class="chip<?php echo $radiusKm === (float) $r ? ' is-active' : ''; ?>">Within <?php echo $r; ?> km</a>
          <?php endforeach; ?>
        </div>
        <a href="<?php echo build_query(['lat' => null, 'lng' => null, 'radius' => null, 'sort' => null, 'page' => null]); ?>" class="clear-location-link">✕ Clear location</a>
      <?php endif; ?>
    </div>
  </section>
  ...
  <section class="results-section">
      ...
      <span class="results-count">
        ...
        <?php if ($categoryKey !== ''): ?>
          in <?php
            ...
            echo htmlspecialchars($activeCatName, ENT_QUOTES);
          ?>
        <?php endif; ?>
      </span>
      ...
      <div class="toolbar-controls">
        ...
        <div class="sort-select">
          ...
          <select id="sortSelect" onchange="window.location.href=this.value">
            <?php if ($hasOrigin): ?>
              <option value="<?php echo build_query(['sort' => 'nearby', 'page' => null]); ?>" <?php echo $sort === 'nearby' ? 'selected' : ''; ?>>Nearest</option>
            <?php endif; ?>
            <option value="<?php echo build_query(['sort' => 'rating', 'page' => null]); ?>" <?php echo $sort === 'rating' ? 'selected' : ''; ?>>Top Rated</option>
            <option value="<?php echo build_query(['sort' => 'price_low', 'page' => null]); ?>" <?php echo $sort === 'price_low' ? 'selected' : ''; ?>>Price: Low to High</option>
            <option value="<?php echo build_query(['sort' => 'price_high', 'page' => null]); ?>" <?php echo $sort === 'price_high' ? 'selected' : ''; ?>>Price: High to Low</option>
            <option value="<?php echo build_query(['sort' => 'newest', 'page' => null]); ?>" <?php echo $sort === 'newest' ? 'selected' : ''; ?>>Newest</option>
          </select>
        </div>

        <?php if (!$dbError && !empty($results)): ?>
          <button type="button" id="viewToggleBtn" class="btn btn-outline btn-sm" data-mode="list">🗺️ Map View</button>
        <?php endif; ?>
      </div>
    </div>
      ...
      <div class="empty-state">
        ...
        <p>
          <?php if ($keyword !== '' || $categoryKey !== ''): ?>
            Try a different keyword or browse all categories instead.
          <?php else: ?>
            No listings yet — check back soon, or be the first to join.
          <?php endif; ?>
        </p>
        <a href="<?php echo build_query(['category' => null, 'q' => null, 'verified' => null, 'page' => null]); ?>" class="btn btn-outline">Browse All Categories</a>
      </div>
    ...
    <?php else: ?>
          ...
          <a href="profile.php?service=<?php echo (int) $row['service_id']; ?>" class="result-card">
            <div class="result-top">
              ...
              <div class="result-who">
                ...
                <span class="result-category"><?php echo htmlspecialchars($row['category_name'], ENT_QUOTES); ?></span>
              </div>
              <?php if ((int) $row['verified_badge'] === 1): ?>
                <span class="verified-tag" title="Verified by SkillBridge.lk">✓</span>
              <?php endif; ?>
            </div>
            ...
            <div class="result-meta">
              ...
              <?php if ($row['distance_km'] !== null): ?>
                <span class="result-distance"><?php echo number_format((float) $row['distance_km'], 1); ?> km away</span>
              <?php endif; ?>
      ...
      <div class="pagination">
        <?php if ($page > 1): ?>
          <a href="<?php echo build_query(['page' => $page - 1]); ?>" class="btn btn-outline">← Previous</a>
        <?php endif; ?>
        <span class="page-indicator">Page <?php echo $page; ?></span>
        <?php if ($hasNextPage): ?>
          <a href="<?php echo build_query(['page' => $page + 1]); ?>" class="btn btn-outline">Next →</a>
        <?php endif; ?>
      </div>
    <?php endif; ?>
